...
form.forEach((item) => checkForm(item));

function checkForm(forma) {
  const errorField = forma.querySelectorAll(".forma__error"),
    btn = forma.querySelector(".forma__btn");
  checkFormField(forma);
  btn.addEventListener("click", (e) => {
    e.preventDefault();
    checkFullField(forma);
    // заявка отправляется при первом же не пустом
    for (error of errorField) if (error.textContent != "") return false;
...
function checkFormField(forma) {
  ...
  const nameInput = forma.querySelector(".name");
  const validateEmail = () => {
    const pattern = /^[-\w.]+@([A-z0-9][-A-z0-9]+\.)+[A-z]{2,4}$/;
    if (emailInput.value.match(pattern)) emailInput.nextElementSibling.innerHTML = "";
  ...
  const validateText = (inp) => {
    ...
    if (inp.value.length == 0)  inp.nextElementSibling.innerHTML = "Поле, обязательное для заполнения";
    else if (inp.value.match(patternLetter) && inp.value.length >= 3) inp.nextElementSibling.innerHTML = "";
    else if (inp.value.length >= 50) inp.nextElementSibling.innerHTML = "Число символов не должно превышать 30";
    else if (inp.value.length < 3) inp.nextElementSibling.innerHTML =  "Число символов не должно быть меньше 3";
    else inp.nextElementSibling.innerHTML = "Поле может содержать только буквы";
  };
  emailInput.addEventListener("change", validateEmail);
  telInput.addEventListener("change", validateTel);
  nameInput.addEventListener("change", () => validateText(nameInput));
}

function checkFullField(forma) {
  const inputRequired = forma.querySelectorAll("[required]");
  for (input of inputRequired) if (!input.value) input.nextElementSibling.innerHTML = "Поле, обязательное для заполнения";
}
